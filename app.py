...
SIGNATURE_PATH = APP_DIR / "employee_signature"
...
def find_saved_signature() -> Path | None:
    for ext in [".png", ".jpg", ".jpeg"]:
        candidate = SIGNATURE_PATH.with_suffix(ext)
        if candidate.exists():
            return candidate
    return None


def save_signature(uploaded_file) -> Path:
    suffix = Path(uploaded_file.name).suffix.lower()
    if suffix not in [".png", ".jpg", ".jpeg"]:
        suffix = ".png"

    for ext in [".png", ".jpg", ".jpeg"]:
        SIGNATURE_PATH.with_suffix(ext).unlink(missing_ok=True)

    destination = SIGNATURE_PATH.with_suffix(suffix)
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        signature_upload = st.file_uploader("Upload / replace employee signature", type=["png", "jpg", "jpeg"])
        if signature_upload:
            saved_signature = save_signature(signature_upload)
            st.success("Employee signature saved for future weeks.")

        if saved_signature and saved_signature.exists():
            try:
                st.image(Image.open(saved_signature), caption="Saved employee signature", width=180)
            except Exception:
    ...
    with st.expander("Advanced: adjust overlay positions", expanded=False):
        st.write("Use this only if your text is slightly off. The original PDF is still used as the background.")
